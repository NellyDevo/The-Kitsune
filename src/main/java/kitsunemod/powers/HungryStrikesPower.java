...
public class HungryStrikesPower extends AbstractKitsunePower {
    public static final String POWER_ID = KitsuneMod.makeID("HungryStrikesPower");
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String NAME = powerStrings.NAME;
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;

    public HungryStrikesPower(AbstractCreature owner, int amount) {
        this.owner = owner;
        name = NAME;
        ID = POWER_ID;
        this.amount = amount;
    ...
    public void onAttack(DamageInfo info, int damage, AbstractCreature target) {
        if (info.type == DamageInfo.DamageType.NORMAL && target != owner && amount > 0) {
